...
<body>
  ...
  'db_password' => '',
				...
				if(is_writable($htaccess_file)){
					echo '<p>.htaccess file is available <i class="fa fa-check-circle-o"></i></p>';
					
				} else {
					echo '<p>.htaccess file is not available <i class="fa fa-times-circle-o"></i></p>';
					$passed = false;
				}
				new_line();
...
<script type="text/javascript" src="application/assets/js/jquery-1.11.1.min.js"></script>
	...
	function step2_complete(){
		...
		$.post("install_setup", { database_host: $('#database_host').val(), database_name: $('#database_name').val(), database_user: $('#database_user').val(), database_password: $('#database_password').val(), }, function(data){
			if(data == 1){
				...
				$('.install_progress').append(progress_error_template('Error: could not add db creds to config'));
				$('#error_btn').fadeIn();
